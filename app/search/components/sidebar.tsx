...
export default function Sidebar({ onFilterChange }: SidebarProps) {
    ...
    return (
        <aside className="">
                ...
                <div className="space-y-3">
                    {categories.map((category) => (
                        <Button
                            ...
                            className={`block text-sm text-left transition-colors ${selectedCategory === category
            ...
            <div>
                <h3 className="text-sm font-medium text-foreground mb-4">Rating</h3>
                <div className="space-y-3">
                    {ratings.map((rating) => (
                        <button
                            key={rating}
                            onClick={() => handleRatingChange(rating)}
                            className={`w-full flex items-center gap-1 text-sm transition-colors p-2 rounded hover:bg-muted ${selectedRating === rating ? 'bg-muted' : ''
                                }`}
                        >
            ...
            {/* Reset Button */}
            <Button variant="outline" className="w-full border-border" onClick={handleReset}>
                Reset
            </Button>
        </aside>
    )
