...
interface ProgressButtonLoadingProps extends React.ComponentPropsWithoutRef<
  typeof Button
> {
  mainText?: string
  state?: boolean
...
function ProgressButtonLoading({
  state = false,
  mainText,
  className,
  loadingText,
  ...props
}: ProgressButtonLoadingProps) {
  return (
    <Button className={cn("relative", className)} disabled={state} {...props}>
      <span
        className={clsx("block font-bold transition ease-in-out", {
          "opacity-0": state,
          "scale-0": state,
        })}
      >
