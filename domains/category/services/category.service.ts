...
async function uniqueSlug(name: string, currentId?: number) {
...
async function validateParent(parentId: number | null, currentId?: number) {
  if (parentId === null) return
  const parent = await repo.findById(parentId)
  if (!parent) throw new Error("Parent category not found")
  if (parent.parentId !== null)
    throw new Error("Categories may only have one level of children")
  if (parent.id === currentId)
    throw new Error("A category cannot be its own parent")
}
function parse(input: unknown): CategoryInput {
  return categoryInputSchema.parse(input)
}
export const getCategories = () => repo.findMany()
export const getCategoryById = async (id: unknown) =>
  repo.findById(categoryIdSchema.parse(id))
...
export async function updateCategory(id: unknown, input: unknown) {
  ...
  if (!(await repo.findById(categoryId))) throw new Error("Category not found")
  const data = parse(input)
  await validateParent(data.parentId, categoryId)
  return repo.update(categoryId, {
    ...data,
    slug: await uniqueSlug(data.name, categoryId),
  })
}
export async function deleteCategory(id: unknown) {
  const categoryId = categoryIdSchema.parse(id)
  if (!(await repo.findById(categoryId))) throw new Error("Category not found")
  if (await repo.hasChildren(categoryId))
    throw new Error("Cannot delete a category with children")
  if (await repo.hasProducts(categoryId))
    throw new Error("Cannot delete a category referenced by products")
...
export async function clearCategories() {
  const repoModule = await import("../repositories/categories.repositories")
  await repoModule.truncateCategories()
}
