...
const iconMap: Record<CategoryIconKey, ComponentType<{
    className?: string;
    stroke?: number;
}>> = {
    ...
    return (
        <section className="w-full">
            <header className="text-center">
                <h1 className="text-3xl font-semibold tracking-tight sm:text-4xl">Explore Categories</h1>
                <p className="mt-2 text-sm text-muted-foreground sm:text-base">Discover curated collections across various industries.</p>
            ...
            <div className="mt-10 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                <div className="relative w-full lg:max-w-xs">
                    ...
                    <Input aria-label="Search categories" className="h-10 bg-card pl-9 text-sm shadow-none" onChange={(event) => setSearchTerm(event.target.value)} placeholder="Search categories..." value={searchTerm} />
                </div>
            ...
            {visibleCategories?.length === 0 && <div className="mt-6 rounded-lg border border-dashed border-border px-6 py-12 text-center text-sm text-muted-foreground">No categories match your search.</div>}
        </section>
    )
}
